...
class ImageSyncService {
    ...
    try {
      ...
      for (int i = 0; i < availableImages.length; i++) {
        ...
        try {
          ...
          final firebaseUrl = await _uploadToFirebase(cardId, format, downloadedImage, _getContentType(url));
      ...
      onProgress(
        ImageSyncProgress(totalImages: 0, completedImages: 0, currentImage: 'Sync failed: $e', status: 'error'),
      );
  ...
  Future<String> _uploadToFirebase(String cardId, String format, Uint8List imageData, String contentType) async {
    final fileName = '$format.${_getFileExtension(contentType)}';
    final path = 'cards/$cardId/images/$fileName';
    final ref = _storage.ref().child(path);

    final uploadTask = ref.putData(imageData, SettableMetadata(contentType: contentType));
